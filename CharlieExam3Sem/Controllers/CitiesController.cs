...
namespace CharlieExam3Sem.Controllers
{
	...
	public class CitiesController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("ZipCode,CityName")] City city)
        {
            if (ModelState.IsValid)
            {
                _context.Add(city);
                await _context.SaveChangesAsync();
                return RedirectToAction(nameof(CitiesList));
            }
            return View(city);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Zip,CityName")] City city)
        {
            if (id != city.ZipCode)
            {
                return NotFound();
